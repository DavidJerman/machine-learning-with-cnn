...
class Application(tk.Frame):
    ...
    def __init__(self, master=None, callbacks=None):
        ...
        self.network = [[[] for x in range(8)] for x in range(50)]
        self.model = None
    ...
    def create_widgets(self):
        ...
        # Output shape info
        self.output_shape_name_label.config(text="Output shape\nbefore flatten:", anchor="center", bg="beige",
                                            font=("courier", 9))
    ...
    def add_layer(self):
        ...
        try:
            ...
            if layer == "Convolution2D":
                self.convolution2d(self.network_position_index, layer)
                self.network_position_index += 1
            elif layer == "MaxPooling2D":
                self.maxpooling2d(self.network_position_index, layer)
                self.network_position_index += 1
            elif layer == "Dropout":
                self.dropout(self.network_position_index, layer)
                self.network_position_index += 1
            elif layer == "Flatten":
                self.flatten(self.network_position_index, layer)
                self.network_position_index += 1
            elif layer == "Dense":
                self.dense(self.network_position_index, layer)
                self.network_position_index += 1
            elif layer == "Input":
                self.input(self.network_position_index, layer)
                self.network_position_index += 1
    ...
    def convolution2d(self, i: int, layer: str):
        """
    ...
    def maxpooling2d(self, i: int, layer: str):
        """
    ...
    def input(self, i: int, layer: str):
        """
        Adds an Input layer
    ...
    def dropout(self, i: int, layer: str):
        """
    ...
    def dense(self, i: int, layer: str):
        """
        Adds a Dense layer
    ...
    def build_and_compile_network(self):
        ...
        if self.optimizer_var.get() == "Adam":
            activation = Adam(lr=self.lr_var.get())
        elif self.optimizer_var.get() == "SGD":
            activation = SGD(lr=self.lr_var.get())
        # Loss function
        loss = None
        ...
        if self.metrics_var.get() == "ACC":
            ...
            metrics = "mae"
        self.model.compile(optimizer=activation, loss=loss, metrics=[metrics])
    ...
    def about(self):
        ...
        messagebox.showinfo(title="About", message="CNN Training GUI\n"
                                                   ...
                                                   " need to work with this program. The program was created for the"
...
class Callbacks(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, batch, logs=None):
        ...
        if not self.stopped_training.get():
            ...
            self.write_output("Loss:     " + str(logs.get('loss')))
            self.write_output("Val loss: " + str(logs.get('val_loss')))
            self.write_output("Acc:      " + str(logs.get('acc')))
            self.write_output("Val acc:  " + str(logs.get('val_acc')))
            self.i += 1
        self.model.stop_training = False
    ...
    def write_output(self, text: str):
        """
        Writes some text in the output_text
